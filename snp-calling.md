...
paste \
   ...
   sort -k3,3 > temp.list
cut -f3 temp.list | uniq -c | sed "s/^ *//g" | tr " " "\t" > unique-counts.list
c=1

# Run counter.sh script
while read i
   do
   bash counter.sh "$i" >> SNP-set
   echo "finished $c"
   c=$(( $c + 1 ))
done < unique-counts.list 
paste temp.list SNP-set | sort -nk1,1 > SNP-set.list
